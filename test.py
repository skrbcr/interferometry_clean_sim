...
if __name__ == '__main__':
    ...
    maskfile = './example/image/structure_mask.png'
    # maskfile = None

    # Set antenna array
    antenna_pos, uv_coverage = clean.set_antenna_array('Y-inclined', 2, b_min=0.1, b_max=0.5, random_seed=0, dt=np.pi / 180, Nt=0, lattitude=np.pi / 6, declination=0, H_0=0)
    ...
    axs[1].scatter(uv_coverage[:, 0], uv_coverage[:, 1], s=1)
    ...
    # # Clean the image
    # psf, model, residual, image = clean.clean(vis, imsize, 'uniform', n_iter=10000, threshold=1e-16, mask=maskfile, gamma=0.2)
